...
static t_listls	*create_file_node(const char *name, const char *full_path)
{
	...
	new_node = malloc(sizeof(t_listls));
	if (!new_node)
		return (NULL);
...
static char	*build_full_path(const char *dir_name, const char *file_name)
{
	...
	if (!str)
		return (NULL);
	full_path = clean_join(str, (char *)file_name);
	return (full_path);
}
...
void	ft_ls(char *name, t_listls **head, t_ls *ls)
{
	...
	while (!is_file && !ls->flags->d && (d = readdir(dir)) != NULL)
	{
		...
		if (d->d_name[0] == '.' && !ls->flags->a)
			continue ;
		full_path = build_full_path(name, d->d_name);
		if (!full_path)
			continue ;
		new_node = create_file_node(d->d_name, full_path);
		if (new_node)
	...
	{
		new_node = create_file_node(name, name);
		if (new_node)
		{
			new_node->next = *head;
			*head = new_node;
